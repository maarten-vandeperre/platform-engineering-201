...
def _assert_github_scaffolder_plugin_loaded(config):
    ...
    installed = (
        "Successfully installed dynamic plugin ./dynamic-plugins/dist/backstage-plugin-scaffolder-backend-module-github-dynamic"
        in logs
    )
    already_present = (
        "Skipping download of already installed dynamic plugin ./dynamic-plugins/dist/backstage-plugin-scaffolder-backend-module-github-dynamic"
        in logs
    )
    assert installed or already_present, (
        "GitHub scaffolder backend plugin is not present in install-dynamic-plugins logs"
    )
